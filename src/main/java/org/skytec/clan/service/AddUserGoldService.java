...
public class AddUserGoldService {
  public void addGoldToClan(long userId, long clanId, long gold) {
    final User user = ctx.userService.get(userId);
    final Clan clan = ctx.clanService.get(clanId);
    ctx.transactionService.commit(clan, user, gold, "replenishment from wallet");
  }
}
